...
__export(db_schema_exports, {
  appUsers: () => appUsers,
  codeLogbook: () => codeLogbook,
  esSystemAuthUser: () => esSystemAuthUser,
  infractionResponseAudit: () => infractionResponseAudit,
  infractions: () => infractions,
  inviteCodes: () => inviteCodes
});
import { sqliteTable, uniqueIndex, text, integer, index } from "drizzle-orm/sqlite-core";
...
var inviteCodes = sqliteTable("invite_codes", {
...
var codeLogbook = sqliteTable("code_logbook", {
  id: integer().primaryKey({ autoIncrement: true }),
...
var infractionResponseAudit = sqliteTable("infraction_response_audit", {
  id: integer().primaryKey({ autoIncrement: true }),
...
__export(db_relations_exports, {
  ...
  infractionResponseAuditRelations: () => infractionResponseAuditRelations,
  infractionsRelations: () => infractionsRelations
});
import { relations } from "drizzle-orm/relations";
var infractionsRelations = relations(infractions, ({ one, many }) => ({
...
var appUsersRelations = relations(appUsers, ({ many }) => ({
...
var infractionResponseAuditRelations = relations(infractionResponseAudit, ({ one }) => ({
...
// src/index.ts
import { eq, and, like, desc, sql as sql2 } from "drizzle-orm";
async function createApp(edgespark) {
  ...
  app.use("*", async (c, next) => {
    ...
    if (user) {
      const appUser = await edgespark.db.query.appUsers.findFirst({
        where: eq(db_schema_exports.appUsers.id, user.id)
      });
  ...
  app.post("/api/public/invites/validate", async (c) => {
    ...
    const invite = await edgespark.db.query.inviteCodes.findFirst({
      where: sql2`lower(${db_schema_exports.inviteCodes.code}) = lower(${code})`
    });
  ...
  app.post("/api/auth/complete-profile", async (c) => {
    ...
    await edgespark.db.insert(db_schema_exports.appUsers).values({
      id: user.id,
      email: user.email,
      fullName,
      role: invite.role,
    ...
    await edgespark.db.update(db_schema_exports.inviteCodes).set({ usedCount: invite.usedCount + 1 }).where(eq(db_schema_exports.inviteCodes.id, invite.id));
  ...
  app.get("/api/system/stats", async (c) => {
    ...
    const allUsers = await edgespark.db.select().from(db_schema_exports.appUsers);
    const stats = {
  ...
  app.get("/api/users/search", async (c) => {
    ...
    if (!query || query.length < 2) {
      let users2 = await edgespark.db.select().from(db_schema_exports.appUsers).limit(100);
      if (roleFilter && roleFilter !== "all") {
        users2 = users2.filter((u) => u.role === roleFilter);
      }
      return c.json({ users: users2 });
    }
    let users = await edgespark.db.select().from(db_schema_exports.appUsers).where(like(db_schema_exports.appUsers.fullName, `%${query}%`)).limit(100);
    if (roleFilter && roleFilter !== "all") {
  ...
  app.get("/api/students/search", async (c) => {
    ...
    const students = await edgespark.db.select().from(db_schema_exports.appUsers).where(and(
      eq(db_schema_exports.appUsers.role, "student"),
      sql2`(
          lower(${db_schema_exports.appUsers.fullName}) LIKE lower(${`%${query}%`}) OR
          lower(${db_schema_exports.appUsers.email}) LIKE lower(${`%${query}%`}) OR
          lower(${db_schema_exports.appUsers.section}) LIKE lower(${`%${query}%`})
        )`
    )).limit(50);
  ...
  app.get("/api/adviser/roster", async (c) => {
    ...
    const students = await edgespark.db.select().from(db_schema_exports.appUsers).where(and(
      eq(db_schema_exports.appUsers.role, "student"),
      eq(db_schema_exports.appUsers.section, appUser.section)
    ));
    return c.json({ students });
  });
  app.post("/api/infractions", async (c) => {
    ...
    const student = await edgespark.db.query.appUsers.findFirst({
      where: eq(db_schema_exports.appUsers.id, studentId)
    });
    if (!student) return c.json({ error: "Student not found" }, 404);
    await edgespark.db.insert(db_schema_exports.infractions).values({
      studentId,
      issuerId: appUser.id,
      type,
      offense,
  ...
  app.get("/api/infractions", async (c) => {
    ...
    if (appUser.role === "student") {
      whereClause = eq(db_schema_exports.infractions.studentId, appUser.id);
    } else if (appUser.role === "parent") {
      return c.json({ infractions: [] });
    } else if (appUser.role === "adviser") {
      if (studentId) {
        whereClause = eq(db_schema_exports.infractions.studentId, studentId);
      } else {
    ...
    const infractions2 = await edgespark.db.query.infractions.findMany({
      where: whereClause,
      orderBy: [desc(db_schema_exports.infractions.dateIssued)],
      limit: Math.min(limit, 500)
    });
    const studentIds = [...new Set(infractions2.map((i) => i.studentId))];
    const issuerIds = [...new Set(infractions2.map((i) => i.issuerId))];
    const allUserIds = [.../* @__PURE__ */ new Set([...studentIds, ...issuerIds])];
    const users = await edgespark.db.query.appUsers.findMany({
      where: sql2`${db_schema_exports.appUsers.id} IN (${sql2.join(allUserIds.map((id) => sql2`${id}`), sql2`, `)})`
    });
    const userMap = new Map(users.map((u) => [u.id, u]));
    let formattedInfractions = infractions2.map((inf) => ({
      id: inf.id,
      type: inf.type,
      offense: inf.offense,
      points: inf.points,
    ...
    if (search && search.length > 0) {
      ...
      formattedInfractions = formattedInfractions.filter(
        (inf) => inf.studentName.toLowerCase().includes(searchLower) || inf.studentSection.toLowerCase().includes(searchLower) || inf.offense.toLowerCase().includes(searchLower)
      );
    }
    if (type && type !== "all") {
  ...
  app.post("/api/infractions/:id/explanation", async (c) => {
    ...
    const infraction = await edgespark.db.query.infractions.findFirst({
      where: eq(db_schema_exports.infractions.id, infractionId)
    });
    if (!infraction) {
    ...
    await edgespark.db.update(db_schema_exports.infractions).set({
      studentExplanation: explanation,
      explanationSubmittedAt: Math.floor(Date.now() / 1e3)
    }).where(eq(db_schema_exports.infractions.id, infractionId));
    return c.json({ success: true });
  });
  app.post("/api/infractions/:id/response", async (c) => {
    ...
    await edgespark.db.update(db_schema_exports.infractions).set({
      ...
      responseSubmittedAt: Math.floor(Date.now() / 1e3)
    }).where(eq(db_schema_exports.infractions.id, infractionId));
    await edgespark.db.insert(db_schema_exports.infractionResponseAudit).values({
      infractionId,
  ...
  app.get("/api/students/:id/points", async (c) => {
    ...
    const infractions2 = await edgespark.db.query.infractions.findMany({
      where: eq(db_schema_exports.infractions.studentId, studentId)
    });
    const activePoints = infractions2.filter((inf) => !inf.resolved).reduce((sum, inf) => sum + inf.points, 0);
    const totalInfractions = infractions2.length;
    const resolvedCount = infractions2.filter((inf) => inf.resolved).length;
    return c.json({
  ...
  app.get("/api/students/:id/profile", async (c) => {
    ...
    if (appUser.role === "adviser") {
      const student2 = await edgespark.db.query.appUsers.findFirst({
        where: eq(db_schema_exports.appUsers.id, studentId)
      });
      if (!student2 || student2.section !== appUser.section) {
        return c.json({ error: "Forbidden" }, 403);
      }
    }
    const student = await edgespark.db.query.appUsers.findFirst({
    ...
    const issuerIds = [...new Set(infractions2.map((i) => i.issuerId))];
    const resolverIds = [...new Set(infractions2.filter((i) => i.resolvedBy).map((i) => i.resolvedBy))];
    const allUserIds = [.../* @__PURE__ */ new Set([...issuerIds, ...resolverIds])];
    const users = await edgespark.db.query.appUsers.findMany({
      where: sql2`${db_schema_exports.appUsers.id} IN (${sql2.join(allUserIds.map((id) => sql2`${id}`), sql2`, `)})`
    });
    const userMap = new Map(users.map((u) => [u.id, u]));
    const formattedInfractions = infractions2.map((inf) => ({
      id: inf.id,
      type: inf.type,
      offense: inf.offense,
      points: inf.points,
    ...
    return c.json({
      ...
      summary: {
        totalInfractions: infractions2.length,
        activePoints,
        resolvedCount: infractions2.filter((i) => i.resolved).length,
        unresolvedCount: infractions2.filter((i) => !i.resolved).length
      }
    });
  });
  app.patch("/api/infractions/:id/resolve", async (c) => {
    ...
    if (appUser.role === "adviser") {
      const student = await edgespark.db.query.appUsers.findFirst({
        where: eq(db_schema_exports.appUsers.id, infraction.studentId)
      });
      if (!student || student.section !== appUser.section) {
        return c.json({ error: "Forbidden" }, 403);
      }
  ...
  app.post("/api/invites/generate", async (c) => {
    ...
    const expiresAt = Math.floor(Date.now() / 1e3) + (expiresInHours || 24) * 3600;
    await edgespark.db.insert(db_schema_exports.inviteCodes).values({
      code,
      role,
      section,
      maxUses: maxUses || 10,
  ...
  app.get("/api/invites/my-codes", async (c) => {
    ...
    const codes = await edgespark.db.query.inviteCodes.findMany({
      where: eq(db_schema_exports.inviteCodes.createdBy, appUser.id),
      orderBy: [desc(db_schema_exports.inviteCodes.createdAt)]
    });
    const now = Math.floor(Date.now() / 1e3);
    const formattedCodes = codes.map((c2) => ({
      id: c2.id,
      code: c2.code,
      role: c2.role,
      section: c2.section,
      maxUses: c2.maxUses,
      usedCount: c2.usedCount,
      expiresAt: c2.expiresAt,
      createdAt: c2.createdAt,
      isExpired: c2.expiresAt < now,
      isFull: c2.usedCount >= c2.maxUses,
      status: c2.expiresAt < now ? "expired" : c2.usedCount >= c2.maxUses ? "full" : "active"
    }));
  ...
  app.delete("/api/invites/:id", async (c) => {
    ...
    const code = await edgespark.db.query.inviteCodes.findFirst({
      where: eq(db_schema_exports.inviteCodes.id, codeId)
    });
    if (!code) {
    ...
    await edgespark.db.delete(db_schema_exports.inviteCodes).where(eq(db_schema_exports.inviteCodes.id, codeId));
    return c.json({ success: true });
  });
  return app;
}
